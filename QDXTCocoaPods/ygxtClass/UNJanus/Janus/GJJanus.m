...
-(void)attachPlugin:(id<GJJanusPluginHandleProtocol>)plugin callback:(AttachCallback)pluginCallback{
    ...
    NSDictionary* r = @{ @"janus": janus_Message[kJanusAttach], @"plugin": plugin.pluginName, @"opaque_id": [NSString stringWithFormat:@"videoroomtest-%@",randomString(12)], @"transaction": transaction,@"session_id":@(_sessionID) };
    NSMutableDictionary* request = [NSMutableDictionary dictionaryWithDictionary:r];
    if (_token != nil) {
        request[@"token"] = _token;
    }
