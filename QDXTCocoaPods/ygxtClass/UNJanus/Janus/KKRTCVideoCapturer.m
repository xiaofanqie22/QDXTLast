    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        self.cropFilter.frameProcessingCompletionBlock = ^(GPUImageOutput *imageOutput, CMTime time) {
            ...
            if (wkSelf.captureCount++ % wkSelf.dropStep.den >= wkSelf.dropStep.num) {
                RTCCVPixelBuffer *rtcPixelBuffer = [[RTCCVPixelBuffer alloc] initWithPixelBuffer:pixel_buffer];
                int64_t timeStampNs = time.value*1000000000/time.timescale;
                RTCVideoFrame *videoFrame = [[RTCVideoFrame alloc] initWithBuffer:rtcPixelBuffer
                                                                         rotation:RTCVideoRotation_0
                                                                      timeStampNs:timeStampNs];
                if(videoFrame != nil){
                    [wkSelf.delegate capturer:wkSelf didCaptureVideoFrame:videoFrame];
                }
