...
- (void)layoutSessions:(NSArray<VideoSession *> *)sessions
    ...
    NSArray *allViews = [self viewListFromSessions:sessions maxCount:100 ignorSession:nil];
    for (int i = 0; i < allViews.count; i ++) {
        UIView * view = allViews[i];
//        UIButton * delBtn = [[UIButton alloc]initWithFrame:CGRectMake(180, 0, 20, 20)];
        UIButton * delBtn = [[UIButton alloc]init];
        delBtn.tag = view.tag;
//        delBtn.tag = sessions.uid;
        [delBtn setBackgroundImage:[UIImage imageNamed:@"del"] forState:UIControlStateNormal];
...
//        delBtn.backgroundColor = [UIColor redColor];
        if (share == YES){
          view.frame = CGRectMake(0, 0, container.frame.size.width, container.frame.size.height);
        }else{
          view.frame = CGRectMake(0, 80 * i + 0, 120, 80);
          
          delBtn.frame = CGRectMake(100, 80 * i , 20, 20);
        }
        if (self.roleIndex == 1) {
...
-(void)delClick:(UIButton * )btn{
    if (self.leftBodyCellDelegate && [self.leftBodyCellDelegate respondsToSelector:@selector(selectedItemButton:)]){
        // 调用代理方法
        [self.leftBodyCellDelegate selectedItemButton:btn.tag];
    }
}
